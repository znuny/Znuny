...
sub SendNotification {
    ...
    if ( $Recipient{Type} eq 'Agent' ) {
        ...
        for my $EnabledParam ( @{$AgentRecipientInformation} ) {
            next ENABLEDPARAMAGENT if $EnabledParam =~ m{\A(?:UserPw|UserPassword)\z};
        ...
        for my $EnabledParam ( @{$CustomerRecipientInformation} ) {
            next ENABLEDPARAMCUSTOMER if $EnabledParam =~ m{\A(?:UserPw|UserPassword)\z};
            $RequestData->{$EnabledParam} = $Recipient{$EnabledParam} // '';
        }
...
sub _SanitizeRecipientData {
    ...
    if ( IsHashRefWithData( $Param{Data} ) ) {
        ...
        for my $Key ( sort keys %{ $Param{Data} } ) {
            ...
            # Note: UserPassword is a configurable customer user field. Only the default field name
            #   'UserPassword' is supported right now.
            if ( $Key =~ m{\A(?:UserPw|UserPassword)\z}i ) {
