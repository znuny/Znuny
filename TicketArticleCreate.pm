...
package Kernel::System::ProcessManagement::TransitionAction::TicketArticleCreateCustom;
...
Kernel::System::ProcessManagement::TransitionAction::TicketArticleCreateCustom - A module to create an article
...
Don't use the constructor directly, use the ObjectManager instead:
    ...
    my $TicketArticleCreateObject = $Kernel::OM->Get('Kernel::System::ProcessManagement::TransitionAction::TicketArticleCreateCustom');
...
##################
        # This patch puts the content of a dynamic field into the body
        # The keyword prefix is "OTRS_TICKET_" folowed by "DynamicField_..."
        my @words = split /OTRS_TICKET_/, $Param{Config}->{Body}, 2;
        if ( $words[1] =~ m/DynamicField_(.*)/ && $Param{Ticket}->{$words[1]} ) {
            # Generate notice body from textfield
            $Param{Config}->{Body} = $Param{Ticket}->{$words[1]};
            # Generate notice subject from ticket title

            my $str = $Param{Ticket}->{$words[1]};
            # Truncate the subject title if greater than 30 chars
            if (length($str) > 30) {
                $str = substr($str,0,30) . " ...";
            }

            $Param{Config}->{Subject} = $Param{Config}->{Subject}.": ".$Param{Ticket}->{Title}.": ".$str;
        }
##################
