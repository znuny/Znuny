...
# get RTE styles that are used in communication
my $MixedWithHTMLCssDefault     = $HTMLUtilsObject->RTEContentCssDefaultFromSysconfigGet() // '';
my $MixedWithHTMLCssInternalRef = $HTMLUtilsObject->RTEContentCssInternalGet(
    UserType => 'Agent',
);

my $InternalStyleIsDefinedScalarRef = $Self->True(
    ( defined $MixedWithHTMLCssInternalRef && ref $MixedWithHTMLCssInternalRef eq 'SCALAR' ),
    'html - internal style is defined with reference to "SCALAR"',
);

my %MultipartMixedHTMLPlain = (
    RTEContentCssDefault  => $MixedWithHTMLCssDefault,
    RTEContentCssInternal => $InternalStyleIsDefinedScalarRef ? ${$MixedWithHTMLCssInternalRef} : '',
);
...
<!DOCTYPE html><html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>' .
                    "<style type=\"text/css\" class=\"RTEContentCssInternal\">$MultipartMixedHTMLPlain{RTEContentCssInternal}</style>"
                    .
                    "<style type=\"text/css\" class=\"RTEContentCssDefault\">$MultipartMixedHTMLPlain{RTEContentCssDefault}</style>"
                    .
                    '</head><body class="ck ck-content"><p>Hello,</p>
<p></p>
...
<p></p>
                ...
                'Filesize'        => 'CalcByContentBytesLength',
                'MimeType'        => 'text/html'
            },
        ],
    },
...
for my $Test (@Tests) {
    ...
    # calculate attachments filesize if it's needed
    for my $Attachment ( @{ $Test->{Attachments} } ) {
        if (
            $Attachment->{Filesize} &&
            $Attachment->{Filesize} eq 'CalcByContentBytesLength'
            )
        {
            $Attachment->{Filesize} = bytes::length( $Attachment->{Content} );
        }
    }
