...
################
        # Patch: Take the ticket title for subject when the specified keyword ist found
        if ( defined $GetParam{Subject} && $Config->{Subject} eq 'OTRS_TICKET_Title' ) {
            $GetParam{Subject}=$Ticket{Title};
        }
################
